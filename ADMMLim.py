...
def compute_x_v_u_ADMM(x_reconstruction_command_line,full_output_path,subdir,i,k,phantom,only_x,subroot_output_path):
    # Compute x,u,v
    os.system(x_reconstruction_command_line)
    # Write x hdr file
    utils.utils_func.write_hdr([i,k+1],subdir,phantom,'x',subroot_output_path=subroot_output_path)
    ...
    utils.utils_func.write_hdr([i,k+1],subdir,phantom,'v',subroot_output_path=subroot_output_path,matrix_type='sino')
    ...
    utils.utils_func.write_hdr([i,k+1],subdir,phantom,'u',subroot_output_path=subroot_output_path,matrix_type='sino')
...
if __name__ == "__main__":
    ...
    suffix =  utils.utils_func.suffix_func(hyperparameters_config) # Suffix to make difference between raytune runs (different hyperparameters)
    ...
    image_init_path_without_extension = hyperparameters_config["image_init_path_without_extension"] # Path to initial image for CASToR ADMM reconstruction
    rho = hyperparameters_config["rho"] # Penalty strength
    alpha = hyperparameters_config["alpha"] # ADMM parameter
    ...
    np.save(subroot + 'Config/hyperparameters_config' + suffix + '.npy', hyperparameters_config)

    # Define PET input dimensions according to input data dimensions
    PETImage_shape_str = utils.utils_func.read_input_dim(subroot+'Data/database_v2/' + phantom + '/' + phantom + '.hdr')
    PETImage_shape = utils.utils_func.input_dim_str_to_list(PETImage_shape_str)

    #Loading Ground Truth image to compute metrics
    image_gt = utils.utils_func.fijii_np(subroot+'Data/database_v2/' + phantom + '/' + phantom + '.raw',shape=(PETImage_shape))
    ...
    compute_x_v_u_ADMM(x_reconstruction_command_line,full_output_path_k_next,subdir,i,k-1,phantom,only_x,subroot_output_path)
    utils.utils_func.write_hdr([i,k+1],'ADMM',phantom,'v',subroot_output_path,matrix_type='sino')

    # Compute one ADMM iteration (x, v, u)
    print('xxxxxxxxxxxxxxxxxxxxx')
    for k in range(-1,hyperparameters_config["nb_iter"]):
        ...
        compute_x_v_u_ADMM(x_reconstruction_command_line,full_output_path_k_next,subdir,i,k,phantom,only_x,subroot_output_path)
