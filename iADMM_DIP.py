...
class iADMM_DIP(vReconstruction):
    ...
    def runComputation(self,config,root):
        ...
        for self.outer_it in range(i_init, self.max_iter):
            ...
            #if (self.outer_it == i_init and not config["unnested_1st_outer_iter"]): # enable to avoid pre iteration
            #    continue # enable to avoid pre iteration

            ####################    Block 1 - Reconstruction with CASToR (tomographic reconstruction part of ADMM)    ####################
            if (self.outer_it != i_init or config["unnested_1st_outer_iter"]): # DIPRecon or DNA after pre iteration
                #if (self.outer_it == i_init + 1 and config["unnested_1st_outer_iter"] == False): # enable to avoid pre iteration
            ...
            # Train network at current outer iteration
            ...
            self.classResults.writeBeginningImages(self.suffix,self.classDenoising.image_net_input_scale,self.outer_it) # Write GT and DIP input
            ...
            if (self.several_DIP_inputs == 1):
                if (self.outer_it == i_init):
                    if (self.classDenoising.SUCCESS):
                        self.f = self.fijii_np(self.subroot_phantom+'Block2/' + self.suffix + '/out_cnn/'+ format(self.experiment)+'/out_' + self.classDenoising.net + '' + format(self.outer_it) + "_epoch=" + format(self.classDenoising.sub_iter_DIP - self.classDenoising.patienceNumber - 1) + '.img',shape=(self.PETImage_shape),type_im='<f') # loading DIP output
                    else:
                        self.f = self.fijii_np(self.subroot_phantom+'Block2/' + self.suffix + '/out_cnn/'+ format(self.experiment)+'/out_' + self.classDenoising.net + '' + format(self.outer_it) + "_epoch=" + format(config["DIP_it_if_no_ES_found"] - 1) + '.img',shape=(self.PETImage_shape),type_im='<f') # loading DIP output
                else:
                    self.f = self.fijii_np(self.subroot_phantom+'Block2/' + self.suffix + '/out_cnn/'+ format(self.experiment)+'/out_' + self.classDenoising.net + '' + format(self.outer_it) + "_epoch=" + format(self.classDenoising.sub_iter_DIP - 1) + '.img',shape=(self.PETImage_shape),type_im='<f') # loading DIP output
                ...
                if (self.outer_it == i_init):
                    ...
                    self.f = self.fijii_np(self.subroot_phantom+'Block2/' + self.suffix + '/out_cnn/'+ format(self.experiment)+'/out_' + self.classDenoising.net + '' + format(self.outer_it) + "_epoch=" + format(self.classDenoising.sub_iter_DIP - 1) + '_batchidx=MR_forward.img',shape=(self.PETImage_shape),type_im='<f') # loading DIP output
            ...
            if (self.outer_it != i_init or config["unnested_1st_outer_iter"]): # DIPRecon after pre iteration
                if (self.outer_it > i_init or ((i_init > -1 and not config["unnested_1st_outer_iter"]) or (i_init > 0 and config["unnested_1st_outer_iter"]))): # at first iteration if rho == 0, let mu to 0 to be equivalent to DIPRecon settings
                    self.mu = self.x_label - self.f
                    self.save_img(self.mu,self.subroot_phantom+'Block2/' + self.suffix + '/mu/'+ format(self.experiment)+'/mu_' + format(self.outer_it) + self.suffix + '.img') # saving mu
                    # Write corrupted image over ADMM iterations
                    self.classResults.writeCorruptedImage(self.outer_it,config["nb_inner_iteration"],self.mu,self.suffix,pet_algo="mmmmmuuuuuuu")
                    print("--- %s seconds - inner_iteration ---" % (time.time() - start_time_inner_iter))
                if (self.simulation):
            ...
            if (self.outer_it != i_init or config["unnested_1st_outer_iter"]): # DIPRecon after pre iteration
                if (self.phantom == "image50_1"):
                    # if (self.classResults.IR_bkg_recon[self.outer_it] > IR_ref[0]):
    ...
    def saveLabel(self,config,i_init):
        if (self.outer_it == i_init and not config["unnested_1st_outer_iter"]): # DIPRecon or DNA at pre iteration -> only pre train the network
