...
def main_computation(config):
    ...
    model = DIP_2D(param1_scale_im_corrupt, param2_scale_im_corrupt, config,'data/Algo/',
                "nested",all_images_DIP="True",global_it=-100, suffix='aaa',last_iter=-1,ground_truth=ground_truth)
    model_class = DIP_2D
...
tune.run(partial(main_computation), config=config_tune,local_dir = os.getcwd() + '/lightning_logs')
# main_computation(config)