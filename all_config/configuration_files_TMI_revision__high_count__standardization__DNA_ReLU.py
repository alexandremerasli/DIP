...
def config_func_MIC():
    ...
    hyperparameters_config = {
        ###########
        ## SIMON
        # ADMM rho associated to the DIP
        "rho" : tune.grid_search([3]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (nested and Gong)    
        # Scaling strategy for the DIP, can choose one or list multiples separated by commas which will run them in parallel
        ...
        # ADMMLim number of inner iterations (do not pay attention to the name of variable, it is actually inner iterations)
        "nb_outer_iteration": tune.grid_search([3]), # Number of outer iterations in ADMMLim (and nested) and OPTITR (for Gong)
        # Mystical line that allows to add the ReLU at the end of the DIP network
        "mu_DIP" : tune.grid_search([1851221]), # Factor to balance primal and dual residual in adaptive alpha computation in ADMMLim
        # Number of iterations for EMV
        ...
        # Alpha EMV to be customized for each algo
        "alpha_EMV" : tune.grid_search([0.04]), # EMV forgetting factor alpha 0.0251
        # Patience number of iterations
        "patienceNumber" : tune.grid_search([350]), # Network to use (DIP,DD,DD_AE,DIP_VAE)
        ###########
    }
