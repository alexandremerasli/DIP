...
def config_func_MIC():
    ...
    settings_config = {
        ...
        "method" : tune.grid_search(['ADMMReg']), # Reconstruction algorithm (DNA, DIPRecon, or algorithms from CASToR (MLEM, BSREM, AML, etc.))
    ...
    fixed_config = {
        ...
        "sub_iter_DIP_initial_and_final" : tune.grid_search([10]), # Number of DIP iterations at DNA/DIPRecon initialization. Could be overrided if early stopping point is reached using "DIP_early_stopping_when" parameter
    ...
    hyperparameters_config = {
        ...
        "rho" : tune.grid_search([3e-5,5e-5,7e-5,2e-4,4e-4,6e-4,8e-4,1e-3]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([3e-5,5e-5,7e-5,2e-4,8e-4,1e-3]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([5e-7,9e-7,4e-6,8e-6,3e-5,5e-5,7e-5,2e-4,8e-4,1e-3]), # QUADRATIC POTENTIAL # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        ...
        "rho" : tune.grid_search([0.05]),
        "rho" : tune.grid_search([0.5,0.3,0.1,0.05,0.03,0.01,0.005,0.003]),
        "rho" : tune.grid_search([0.1,0.05,0.03,0.01,0.005,0.003]), # FDG low count

        # "rho" : tune.grid_search([5e-7,9e-7,4e-6,8e-6]), # QUADRATIC POTENTIAL # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        ...
        "nb_inner_iteration": tune.grid_search([1000]), # Number of inner iterations in DNA and DIPRecon (respectively number of iterations of ADMM-Reg and OPTITR)
        ...
        "A_AML" : tune.grid_search([-10,-100,-1000]), # AML lower bound A
