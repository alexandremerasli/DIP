...
def config_func_MIC():
    ...
    settings_config = {
        ...
        "method" : tune.grid_search(['APGMAP']), # Reconstruction algorithm (DNA, DIPRecon, or algorithms from CASToR (MLEM, BSREM, AML, etc.))
    ...
    hyperparameters_config = {
        ...
        # "rho" : tune.grid_search([1e-3]), # QUADRATIC POTENTIAL # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        #"rho" : tune.grid_search([3e-5]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        ...
        "rho" : tune.grid_search([3e-6,3e-5]), # 90Y
        ...
        "rho" : tune.grid_search([0.5,0.3,0.1,0.05,0.03,0.01]), # FDG brain 2D
        ...
        "nb_outer_iteration": tune.grid_search([10000]), # Number outer iterations in ADMMReg
        ...
        "mu_adaptive" : tune.grid_search([10]), # Factor to balance primal and dual residual in adaptive alpha computation in ADMMReg
