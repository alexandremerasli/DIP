...
def config_func_MIC():
    ...
    fixed_config = {
        ...
        "sub_iter_DIP_init" : tune.grid_search([400]), # Number of DIP iterations at DNA/DIPRecon initialization. Could be overrided if early stopping point is reached using "DIP_early_stopping_when" parameter
        ...
        "alpha_EMV" : tune.grid_search([0.0251*5]), # EMV forgetting factor alpha
        "windowSize" : tune.grid_search([50]), # WMV window size
        "patienceNumber" : tune.grid_search([40]), # Patience number in moving variance algorithms
    }
    ...
    hyperparameters_config = {
        ...
        "rho" : tune.grid_search([3,0.3]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([3]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([3,0.3,0.03,0.003]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        ...
        "tau_DIP" : tune.grid_search([200]), # Factor to multiply alpha in adaptive alpha computation in ADMMReg. If adaptive tau, it corresponds to tau max
        ...
        "sub_iter_DIP" : tune.grid_search([10,30]), # Number of epochs in network optimization
        ...
        "nb_inner_iteration": tune.grid_search([2]), # Number of inner iterations in DNA and DIPRecon (respectively number of iterations of ADMM-Reg and OPTITR)
