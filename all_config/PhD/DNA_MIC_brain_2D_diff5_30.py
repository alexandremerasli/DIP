...
def config_func_MIC():
    ...
    fixed_config = {
        "max_iter" : tune.grid_search([300]), # Number of global iterations for usual optimizers (MLEM, BSREM, AML etc.) and for DNA and DIPRecon
        ...
        "sub_iter_DIP_initial_and_final" : tune.grid_search([400]), # Number of epochs in first global iteration (pre iteraiton) in network optimization (only for DIPRecon for now)
    ...
    hyperparameters_config = {
        ...
        "nb_outer_iteration": tune.grid_search([2]), # Number of outer iterations in ADMMReg (and DNA) and OPTITR (for DIPRecon)
