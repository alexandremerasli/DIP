...
def config_func_MIC():
    ...
    settings_config = {
        "image" : tune.grid_search(['image4_0']), # Image from database (data/Algo/Data/database_v2)
    ...
    fixed_config = {
        "max_iter" : tune.grid_search([700]), # Number of iterations for usual optimizers (MLEM, BSREM, AML etc.) and outer iterations for DNA and DIPRecon
        ...
        "sub_iter_DIP_init" : tune.grid_search([2000]), # Number of DIP iterations at DNA/DIPRecon initialization. Could be overrided if early stopping point is reached using "DIP_early_stopping_when" parameter
        ...
        "patienceNumber" : tune.grid_search([200]), # Patience number in moving variance algorithms
    }
    ...
    hyperparameters_config = {
        ...
        "rho" : tune.grid_search([0.003,0.3,0.03,0.0003,0.00003]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        # "rho" : tune.grid_search([0.00003,0.0003]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        # "rho" : tune.grid_search([3e-4,3e-5,3e-1,3e-2]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        # "rho" : tune.grid_search([0.3,0.03,0.0003,0.00003]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([0.3,0.03,0.003,3e-4]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([0.3,0.03,0.003,3e-4,3e-5]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([0.3,0.003,3e-5]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([300,10,3,1]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([3,0.3,0.03,0.003,3e-4,3e-5]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([3000,300,30,3e-6,3e-7,3e-8]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        "rho" : tune.grid_search([0.03,0.3,3e-5]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)    
        ...
        "mu_DIP" : tune.grid_search([100]), # Factor to balance primal and dual residual in adaptive alpha computation in ADMMReg
        "tau_DIP" : tune.grid_search([0.95,0.92,0.9,0.8]), # Factor to multiply alpha in adaptive alpha computation in ADMMReg. If adaptive tau, it corresponds to tau max
        ...
        "nb_inner_iteration": tune.grid_search([2,10]), # Number of inner iterations in DNA and DIPRecon (respectively number of iterations of ADMM-Reg and OPTITR)
        "nb_inner_iteration": tune.grid_search([10,30,100]), # Number of inner iterations in DNA and DIPRecon (respectively number of iterations of ADMM-Reg and OPTITR)
        "nb_inner_iteration": tune.grid_search([10,30,100]), # Number of inner iterations in DNA and DIPRecon (respectively number of iterations of ADMM-Reg and OPTITR)
        "nb_inner_iteration": tune.grid_search([10,30]), # Number of inner iterations in DNA and DIPRecon (respectively number of iterations of ADMM-Reg and OPTITR)
        "nb_inner_iteration": tune.grid_search([1,10,30]), # Number of inner iterations in DNA and DIPRecon (respectively number of iterations of ADMM-Reg and OPTITR)
