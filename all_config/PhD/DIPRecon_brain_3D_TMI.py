...
def config_func_MIC():
    ...
    settings_config = {
        ...
        "processing_unit" : tune.grid_search(['GPU']), # Run NN training with pytorch on CPU or GPU
        "nb_threads" : tune.grid_search([60]), # Number of desired threads in reconstruction with CASToR. 0 means all the available threads will be used
    ...
    fixed_config = {
        ...
        "sub_iter_DIP_init" : tune.grid_search([5000]), # Number of DIP iterations at DNA/DIPRecon initialization. Could be overrided if early stopping point is reached using "DIP_early_stopping_when" parameter
        ...
        "alpha_EMV" : tune.grid_search([0.01,0.0251,0.1,0.5,0.9,0.99]), # EMV forgetting factor alpha
        "alpha_EMV" : tune.grid_search([0.0251]), # EMV forgetting factor alpha
        "windowSize" : tune.grid_search([50]), # WMV window size
        "patienceNumber" : tune.grid_search([1000]), # Patience number in moving variance algorithms
    }
    ...
    hyperparameters_config = {
        ...
        "rho" : tune.grid_search([1e-6]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        ...
        "mu_DIP" : tune.grid_search([100.1]), # Factor to balance primal and dual residual in adaptive alpha computation in ADMMReg
