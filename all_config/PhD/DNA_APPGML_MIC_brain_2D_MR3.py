...
def config_func_MIC():
    ...
    fixed_config = {
        "max_iter" : tune.grid_search([50]), # Number of iterations for usual optimizers (MLEM, BSREM, AML etc.) and outer iterations for DNA and DIPRecon
    ...
    hyperparameters_config = {
        ...
        "rho" : tune.grid_search([0.3]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
        ...
        "mu_DIP" : tune.grid_search([10.1]), # Factor to balance primal and dual residual in adaptive alpha computation in ADMMReg
        ...
        "alpha" : tune.grid_search([0.01]), # alpha (penalty parameter) in ADMMReg
