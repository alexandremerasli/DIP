...
def config_func_MIC():
    ...
    settings_config = {
        ...
        "nb_threads" : tune.grid_search([7]), # Number of desired threads. 0 means all the available threads
        ...
        "castor_foms" : tune.grid_search([False]), # Set to True to compute CASToR Figure Of Merits (likelihood, residuals for ADMMReg)
    }
    # Configuration dictionnary for previous hyperparameters, but fixed to simplify
    fixed_config = {
        "max_iter" : tune.grid_search([20]), # Number of global iterations for usual optimizers (MLEM, BSREM, AML etc.) and for DNA and DIPRecon
