        ...
        for ROI in ['hot','cold']:
            ...
            for method in method_list: # Loop over methods
                ...
                for fig_nb in range(3):
                    ...
                    for rho_idx in range(nb_rho):
                        ...
                        if (fig_nb == 0):
                            ...
                            for it in idx_sort:
                                ...
                                print(it)
                                print(IR_final_array[rho_idx].shape)
                                print(IR_final_array[rho_idx][:,it])
                                print(metrics_final_array[rho_idx].shape)
                                print(metrics_final_array[rho_idx][:,it])
                    ...
                    IR_final_final_array = np.zeros((nb_rho,nb_replicates,np.min(len_mini)))
                    metrics_final_final_array = np.zeros((nb_rho,nb_replicates,np.min(len_mini)))
                    for rho_idx in range(nb_rho):
                        for common_it in range(np.min(len_mini)):
                            for idx_replicate in range(nb_replicates):
                                IR_final_final_array[rho_idx,idx_replicate,common_it] = IR_final_array[rho_idx][idx_replicate,common_it]
                                metrics_final_final_array[rho_idx,idx_replicate,common_it] = metrics_final_array[rho_idx][idx_replicate,common_it]

                    
                    for rho_idx in range(nb_rho):
                        # Compute average of tradeoff and bias curves with iterations
                        avg_metrics.append(np.sum(metrics_final_final_array[rho_idx],axis=0) / nb_replicates)
                        avg_IR.append(np.sum(IR_final_final_array[rho_idx],axis=0) / nb_replicates)
                        # Compute std bias curves with iterations
                        std_metrics.append(np.sqrt(np.sum((metrics_final_final_array[rho_idx,:] - np.array(avg_metrics)[rho_idx,:])**2,axis=0) / nb_replicates))
                        std_IR.append(np.sqrt(np.sum((IR_final_final_array[rho_idx,:]- np.array(avg_IR)[rho_idx,:])**2,axis=0) / nb_replicates))
