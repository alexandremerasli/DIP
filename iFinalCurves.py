            ...
            for method in method_list: # Loop over methods
                ...
                for fig_nb in range(3):
                    ...
                    for rho_idx in range(nb_rho):
                        ...
                        if (fig_nb == 0):
                            ...
                            for it in idx_sort:
                                ...
                                print(100*np.array(IR_final)[cases,it])
                                reg[fig_nb][rho_idx].append(self.linear_regression(100*np.array(IR_final)[cases,it],np.array(metrics_final)[cases,it]))
                                print(self.linear_regression(100*np.array(IR_final)[cases,it],np.array(metrics_final)[cases,it]))
                    ...
                    if (fig_nb == 2): # Plot tradeoff curves at convergence
                        for rho_idx in range(nb_rho):
                            cases = np.arange(nb_replicates*rho_idx,nb_replicates*(rho_idx+1))
                            reg[fig_nb].append(self.linear_regression(100*np.array(IR_final)[cases,-1],np.array(metrics_final)[cases,-1]))
