...
class iNestedADMM(vReconstruction):
    ...
    def runComputation(self,config,fixed_config,hyperparameters_config,root):
        ...
        for i in range(self.max_iter):
            ...
            self.f = fijii_np(self.subroot+'Block2/out_cnn/'+ format(self.experiment)+'/out_' + classDenoising.net + '' + format(i) + self.suffix + '.img',shape=(self.PETImage_shape)) # loading DIP output

            # Block 3 - mu update
            self.mu += self.x_label - self.f
            save_img(self.mu,self.subroot+'Block2/mu/'+ format(self.experiment)+'/mu_' + format(i) + self.suffix + '.img') # saving mu
            # Write corrupted image over ADMM iterations
