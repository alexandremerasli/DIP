    ...
    def runComputation(self,config,fixed_config,hyperparameters_config,root):
        ...
        for i in range(i_init, self.max_iter):
            ...
            if (i != 0 or fixed_config["method"] != "Gong"): # Gong at first epoch -> only pre train the network
                ...
                self.x_label = self.castor_reconstruction(classResults.writer, i, self.subroot, self.nb_inner_iteration, self.experiment, hyperparameters_config, self.method, self.phantom, self.replicate, self.suffix, classResults.image_gt, self.f, self.mu, self.PETImage_shape, self.PETImage_shape_str, self.rho, self.alpha, self.image_init_path_without_extension) # without ADMMLim file
                # Write corrupted image over ADMM iterations
                classResults.writeCorruptedImage(i,hyperparameters_config["nb_inner_iteration"],self.x_label,self.suffix,pet_algo="nested ADMM")
            ...
            if (i != 0 or fixed_config["method"] != "Gong"): # Gong at first epoch -> only pre train the network
                ...
                # Write corrupted image over ADMM iterations
                classResults.writeCorruptedImage(i,hyperparameters_config["nb_inner_iteration"],self.mu,self.suffix,pet_algo="mmmmmuuuuuuu")
                ...
                classResults.writeEndImagesAndMetrics(i,hyperparameters_config["nb_inner_iteration"],self.PETImage_shape,self.f,self.suffix,self.phantom,classDenoising.net,pet_algo=fixed_config["method"])
