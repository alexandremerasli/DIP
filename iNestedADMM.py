...
class iNestedADMM(vReconstruction):
    ...
    def runComputation(self,config,config3,config4,config2,root):
        ...
        if (config4["unnested_1st_global_iter"]):
            i_init = 0
        else:
            i_init = -1
        ...
        for i in range(i_init, self.max_iter):
            ...
            if (i != i_init or config4["unnested_1st_global_iter"]): # Gong at first epoch -> only pre train the network
                # Block 1 - Reconstruction with CASToR (tomographic reconstruction part of ADMM)
                self.x_label = self.castor_reconstruction(classResults.writer, i, self.subroot, config2["nb_outer_iteration"], self.experiment, config2, self.method, self.phantom, self.replicate, self.suffix, classResults.image_gt, self.f, self.mu, self.PETImage_shape, self.PETImage_shape_str, self.alpha, self.image_init_path_without_extension) # without ADMMLim file
                # Write corrupted image over ADMM iterations
                classResults.writeCorruptedImage(i,config2["nb_outer_iteration"],self.x_label,self.suffix,pet_algo="nested ADMM")

            # Block 2 - CNN
            start_time_block2= time.time()
            if (i == i_init and not config4["unnested_1st_global_iter"]): # Gong at first epoch -> only pre train the network
                ...
                if (config3["method"] == "nested"): # Nested needs 1 to not add any prior information at the beginning, and to initialize x computation to uniform with 1
                    x_label = self.fijii_np(self.subroot_data + 'Data/initialization/' + 'MLEM_it60_REF_cropped.img',shape=(self.PETImage_shape),type='<f')
                elif (config3["method"] == "Gong"): # Fit MLEM 60it for first global iteration
                    x_label = self.fijii_np(self.subroot_data + 'Data/initialization/' + 'MLEM_it60_REF_cropped.img',shape=(self.PETImage_shape),type='<f')
            ...
            '''
            if (not config4["unnested_1st_global_iter"]): # Rho is not zero, so initialize the network with f_init
            ...
            classDenoising = iDenoisingInReconstruction(config2,i)
            ...
            if (i != i_init or config4["unnested_1st_global_iter"]): # Gong at first epoch -> only pre train the network
                ...
                # Write corrupted image over ADMM iterations
                classResults.writeCorruptedImage(i,config2["nb_outer_iteration"],self.mu,self.suffix,pet_algo="mmmmmuuuuuuu")
                ...
                classResults.writeEndImagesAndMetrics(i,config2["nb_outer_iteration"],self.PETImage_shape,self.f,self.suffix,self.phantom,classDenoising.net,pet_algo=config3["method"])
            ...
            classResults.WMV_plot(config4, config2)
