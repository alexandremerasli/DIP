...
settings_config = {
    "image" : tune.grid_search(['image3_10']), # Image from database (data/Algo/Data/database_v2)
    ...
    # "replicates" : tune.grid_search(list(range(1,50+1))), # List of desired replicates to work with in parallel. list(range(1,n+1)) means n replicates
...
fixed_config = {
    "max_iter" : tune.grid_search([10]), # Number of iterations for usual optimizers (MLEM, BSREM, AML etc.) and outer iterations for DNA and DIPRecon
...
hyperparameters_config = {
    ...
    "mu_DIP" : tune.grid_search([120]), # Factor to balance primal and dual residual in adaptive alpha computation in ADMMReg
    "tau_DIP" : tune.grid_search([1.5]), # Factor to multiply alpha in adaptive alpha computation in ADMMReg. If adaptive tau, it corresponds to tau max
    ...
    "nb_inner_iteration": tune.grid_search([3000]), # Number of inner iterations in DNA and DIPRecon (respectively number of iterations of ADMM-Reg and OPTITR)
...
for method in config["method"]['grid_search']:
    ...
    elif ((method == "DIPRecon" or method == "DNA") and config["rho"]["grid_search"][0] == 0 and task != "post_reco" and task != "show_results_post_reco"):
