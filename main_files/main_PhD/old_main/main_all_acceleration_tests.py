...
settings_config = {
    ...
    "image" : tune.grid_search(['image50_1']), # Image from database (data/Algo/Data/database_v2)
    "random_seed" : tune.grid_search([True]), # If True, random seed is used for reproducibility (must be set to False to vary weights initialization)
    "method" : tune.grid_search(["DNA_MIC_brain_2D_random0","DNA_MIC_brain_2D_random1","DNA_MIC_brain_2D_random2","DNA_MIC_brain_2D_random3","DNA_MIC_brain_2D_MR3"]), # Reconstruction algorithm (DNA, DIPRecon, or algorithms from CASToR (MLEM, BSREM, AML, etc.))
    ...
    "replicates" : tune.grid_search(list(range(1,20+1))), # List of desired replicates to work with in parallel. list(range(1,n+1)) means n replicates
