...
# parser.add_argument('--replicate', type=int, dest='replicate', help='replicate id')
...
settings_config = {
    "image" : tune.grid_search(['image010_3D']), # Image from database (data/Algo/Data/database_v2)
    ...
    # "replicates" : tune.grid_search([args.replicate]), # List of desired replicates to work with in parallel. list(range(1,n+1)) means n replicates
...
fixed_config = {
    ...
    "patienceNumber" : tune.grid_search([300]), # Patience number in moving variance algorithms
}
...
hyperparameters_config = {
    ...
    "rho" : tune.grid_search([0.03,0.003,0.0003]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
    ...
    "mu_DIP" : tune.grid_search([50]), # Factor to balance primal and dual residual in adaptive alpha computation in ADMMReg
    "tau_DIP" : tune.grid_search([50]), # Factor to multiply alpha in adaptive alpha computation in ADMMReg. If adaptive tau, it corresponds to tau max
    ...
    "override_it_DIP_with_EMV_it" : tune.grid_search([True]), # Number of epochs in network optimization
...
for method in config["method"]['grid_search']:
    ...
    if (method == "DIPRecon"):
        config_tmp["nb_inner_sub_iteration"]['grid_search'] = [50]
        #config_tmp["lr"]['grid_search'] = [0.5]
        #config_tmp["rho"]['grid_search'] = [0.0003]
        config_tmp["lr"]['grid_search'] = [0.5]
        config_tmp["rho"]['grid_search'] = [0.0003]
    elif (method == 'DNA'):
        config_tmp["nb_inner_sub_iteration"]['grid_search'] = [10]
...
if (task != "post_reco" and "3D" not in config["image"]["grid_search"][0]):
