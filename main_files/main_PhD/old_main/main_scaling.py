...
settings_config = {
    "image" : tune.grid_search(['image50_2']), # Image from database (data/Algo/Data/database_v2)
    "image" : tune.grid_search(['image4_1']), # Image from database (data/Algo/Data/database_v2)
    "random_seed" : tune.grid_search([True]), # If True, random seed is used for reproducibility (must be set to False to vary weights initialization)
    "image" : tune.grid_search(['image40_1']), # Image from database (data/Algo/Data/database_v2)
    "image" : tune.grid_search(['image4_0']), # Image from database (data/Algo/Data/database_v2)
    
    
    "method" : tune.grid_search(["DNA_ADMMReg_more_ADMMReg_it_10","DNA_APPGML_1it","DIPRecon_skip3_3_my_settings","DIPRecon_CT_1_skip"]), # Reconstruction algorithm (DNA, DIPRecon, or algorithms from CASToR (MLEM, BSREM, AML, etc.))
    "method" : tune.grid_search(["DNA_APPGML_1it"]), # Reconstruction algorithm (DNA, DIPRecon, or algorithms from CASToR (MLEM, BSREM, AML, etc.))
    
    "method" : tune.grid_search(['DIPRecon_skip3_3_my_settings','DIPRecon_CT_1_skip']), # Reconstruction algorithm (DNA, DIPRecon, or algorithms from CASToR (MLEM, BSREM, AML, etc.))
    # "method" : tune.grid_search(['DIPRecon_skip3_3_my_settings']), # Reconstruction algorithm (DNA, DIPRecon, or algorithms from CASToR (MLEM, BSREM, AML, etc.))
