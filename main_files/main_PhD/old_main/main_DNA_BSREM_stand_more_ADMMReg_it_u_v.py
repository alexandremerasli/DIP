...
settings_config = {
    ...
    #"replicates" : tune.grid_search([2,19,24,30,32,39,43,51,53,56,61,66,77]), # List of desired replicates to work with in parallel. list(range(1,n+1)) means n replicates
    #"replicates" : tune.grid_search([1,2,3,8,9,10,11,12,15,18,19,20,22,27,28,30,33,34,35,38,40]), # List of desired replicates to work with in parallel. list(range(1,n+1)) means n replicates
    #"replicates" : tune.grid_search([9,19]), # List of desired replicates to work with in parallel. list(range(1,n+1)) means n replicates
    #"replicates" : tune.grid_search([1,3,4,13,22,24,28,30,33,39,43]), # List of desired replicates to work with in parallel. list(range(1,n+1)) means n replicates
    #"replicates" : tune.grid_search([19,24,28,30,32,39,43,51,53,56,61,66,77,78,86]), # List of desired replicates to work with in parallel. list(range(1,n+1)) means n replicates
...
fixed_config = {
    ...
    "use_u_and_v_DNA" : tune.grid_search([True]), # For DNA reconstruction, set to True to initialize current sinograms u and v by those from previous outer iteration
