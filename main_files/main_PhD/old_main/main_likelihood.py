...
hyperparameters_config = {
    ...
    "rho" : tune.grid_search([1]), # Penalty strength (beta) in PLL algorithms, ADMM penalty parameter (DNA and DIPRecon)
    ...
    "tau_DIP" : tune.grid_search([0.92]), # Factor to multiply alpha in adaptive alpha computation in ADMMReg. If adaptive tau, it corresponds to tau max
...
for method in config["method"]['grid_search']:
    ...
    elif (task == 'compute_likelihood'): # Compute likelihood with simple CASToR command line just to compute FOMs on given initialization image
        from iComputeLikelihood_DNA import iComputeLikelihood_DNA
        classTask = iComputeLikelihood_DNA(config)
