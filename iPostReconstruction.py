...
class iPostReconstruction(vDenoising):
    ...
    def runDenoiser(self):
        ...
        write_image_tensorboard(self.writer,self.image_corrupt,"Corrupted image to fit",suffix_func(self.config),self.image_gt) # Showing corrupted image
        write_image_tensorboard(self.writer,self.image_corrupt,"Corrupted image to fit (FULL CONTRAST)",suffix_func(self.config),self.image_gt,0,full_contrast=True) # Showing corrupted image
        for epoch in range(0,self.max_iter,self.max_iter//10):      
            write_image_tensorboard(self.writer,self.image_net_input,"DIP input (FULL CONTRAST)",suffix_func(self.config),self.image_gt,epoch,full_contrast=True) # DIP input in tensorboard
            if (epoch > 0):
                ...
                compute_metrics(self.PETImage_shape,out_descale,self.image_gt,epoch,self.PSNR_recon,self.PSNR_norm_recon,self.MSE_recon,self.MA_cold_recon,self.CRC_hot_recon,self.CRC_bkg_recon,self.IR_bkg_recon,self.phantom,writer=self.writer,write_tensorboard=True)
                # Saving (now DESCALED) image output
                save_img(out_descale, net_outputs_path)

            # Write images over epochs
            write_image_tensorboard(self.writer,out_descale,"Image over epochs (" + self.net + "output)","",self.image_gt,epoch) # Showing all images with same contrast to compare them together
            write_image_tensorboard(self.writer,out_descale,"Image over epochs (" + self.net + "output, FULL CONTRAST)","",self.image_gt,epoch,full_contrast=True) # Showing each image with contrast = 1
            self.writer.close()