...
class iResults(vDenoising):
    ...
    def runComputation(self,config,fixed_config,hyperparameters_config,root):
        ...
        for i in range(self.max_iter):
            f = fijii_np(subroot+'Block2/out_cnn/'+ format(self.experiment)+'/out_' + self.net + '' + format(i) + self.suffix + '.img',shape=(self.PETImage_shape)) # loading DIP output

            # Write images over epochs
            self.writeEndImages(i,self.max_iter,self.PETImage_shape,f,self.suffix,self.phantom,self.net,pet_algo="to fit",iteration_name="(post reconstruction)")
